...
Bonn <- read_csv2(paste0(wdr,"/data/OHRDecisions.csv")) %>% 
  rename(area=category_name)

OHR <- readxl::read_xlsx("data/general_data.xlsx", sheet="OHRMandates") %>% 
  mutate_at(vars(contains("date")), as.Date)

Gov <- readxl::read_xlsx("data/general_data.xlsx", sheet="BiHGovs") %>% 
  mutate_at(vars(start, contains("end")), as.Date) %>% 
  filter(!is.na(end.effective))
...
Bonn <-  Bonn %>%
  mutate(area2=case_when(str_detect(decision.name, regex("lift", ignore_case=T)) &
                           str_detect(decision.name, regex("ban", ignore_case=T))  ~ "Lifted Removals and Suspensions from Office",
                                   TRUE ~ as.character(area))) %>% 
  mutate(year=lubridate::year(date.publish))
table(Bonn$area2, useNA = c("always"))
...
Bonn.year.plot.all <- Bonn.year %>%
  filter(area2 %in% c("Lifted Removals and Suspensions from Office",
                     "Removals and Suspensions from Office")) %>%
  spread(key=area2, value=freq, fill=0, drop=FALSE)%>%
  gather(key=area2, value=freq, -year )%>%
  ggplot(.,aes(year, freq))+
  geom_bar(aes(fill=area2),stat="identity",position="dodge")+
  ...
  guides(fill=guide_legend(keywidth = 2, keyheight = 0.7))+
    scale_x_continuous(limit=c(1995, 2015), 
                       breaks=seq(1995,2015), labels=substring(as.character(seq(1995,2015)),3))+
...
# >>> Bar Plot Remoovals and Const issues ------------------------------------------------------------
...
Bonn.year.plot <- Bonn.year %>%
  filter(area2 %in% areas.selected) %>%
  ggplot(.,aes(year, freq))+
...
Bonn.year.facet.inst <- Bonn.year %>%
  filter(area2 %in% c("State Symbols, State-Level Matters and Const. Issues",
                      "Federation, Mostar and H-N Canton",
                      "Judicial Reform")) %>%
  ggplot(.,aes(year, freq))+
  # geom_rect(data=subset(Bonn.year,area %in% c(),
...
tab1<- formattable(Bonn.HR.tab, align=allign.columns, 
             ...
             list(area(col=8, row=1:9) ~ color_bar("orange"),
                  ...
                  area(col=2:7, row=9) ~ color_bar("orange"),
...
# > Number and categories of Bonn P per OHR - stacked bar ------------------
...
Bonn.HR.col <- Bonn.HR %>%
  ...
  guides(fill=guide_legend(ncol=2, keywidth = 0.3, keyheight=0.3, reverse = F))+
  coord_flip()
...
#time <- format(Sys.time(),"%Y%m%d-%H%M%S")
filename <-"-BonnHR.png"
ggsave(paste0(wdr, folder,time, filename), width=16, height=7, unit="cm")
...
Bonn.week.tab <- Bonn.week

names(Bonn.week.tab) <- c("HR","number of decisions", "mandate in weeks", "label", "weekly ratio")

Bonn.week.tab <- Bonn.week.tab %>% 
  #select(-label) %>% 
...
tab1<- formattable(Bonn.week.tab, align=allign.columns, 
...
browsable(
  tagList(
    tags$div(
      tags$h3(class="strong", "Bonn Power decisions per HR and per week")),
      #tags$h4("Data for Inzko right censored with Dec 2014", style="color:grey" )),
    as.htmlwidget(tab1),
    tags$div(align="right",
      tags$small("graph: author: data: OHR.int", sttyle="color:grey"))
  )
)->tab1.all
...
# 1. Open jpeg file
png("Week.Bonn.plot1.png", width = 700, height = 500, units="px")
tab1.all
dev.off()
...
export_formattable(tab1.all,"graphs/Bonn.weekly.png")
saveWidget(tab1.all)
...
Bonn.week.plot <- Bonn.week %>%
  ...
  geom_bar(fill="#4C566A", stat="identity")+
...
plotname <-"Bonn_week_plot.png"
ggsave(paste0(wdr, folder,time,scriptname,plotname), width=15, height=9, unit="cm")
...
Bonn.joy <- Bonn %>%
  mutate(year=lubridate::year(date.publish)) %>% 
  group_by(area2, year, HR)%>%
  summarise(freq=n())#%>%
  spread(key=area2, value=freq, fill=0, drop=FALSE) %>%
  gather(key=area2, value=freq, 3:8)

#Bonn.joy <- Bonn

Bonn.joy$phase <- NA
Bonn.joy$phase[Bonn.joy$area2 == "Lifted Removals and Suspensions from Office"] <- "Means to sanction"
Bonn.joy$phase[Bonn.joy$area2 == "Removals and Suspensions from Office"] <- "Means to sanction"
Bonn.joy$phase[Bonn.joy$area2 == "Economic Field"] <- "Means to govern"
Bonn.joy$phase[Bonn.joy$area2 == "State Symbols, State-Level Matters and Const. Issues"] <- "Means for state-building and agreement recalibration"
Bonn.joy$phase[Bonn.joy$area2 == "Judicial Reform"] <- "Means for state-building and agreement recalibration"
Bonn.joy$phase[Bonn.joy$area2 == "Federation, Mostar and H-N Canton"] <- "Means for state-building and agreement recalibration"
Bonn.joy$phase[Bonn.joy$area2 == "Media Restructuring Decisions"] <- "Means to govern"
Bonn.joy$phase[Bonn.joy$area2 == "Property Laws, Return of Displaced Persons and Refugees and Reconciliation"] <- "Means to govern"
Bonn.joy$phase[Bonn.joy$area2 == "Individuals indicted for War Crimes in the former Yugoslavia"] <- "Means to govern"
Bonn.joy$phase <- factor(Bonn.joy$phase)

Bonn.joy <- Bonn.joy %>% 
  ungroup() %>% 
  mutate(phase=forcats::fct_rev(phase)) %>% 
  mutate(area2 = area2 %>% as_factor) %>% 
  mutate(area2=forcats::fct_relevel(area2, levels=c("Removals and Suspensions from Office", 
...
Bonn.joy <- Bonn.joy %>% 
  mutate(HR=HR %>% forcats::as_factor())
...
table(Bonn.joy$phase, Bonn.joy$area2, useNA = c("always"))
...
mandate.colors <- c("Westendorp"="#8B475D", 
                    "Petritsch"="#CD7054", 
                    "Inzko"="#EE9A49",
                    "Schwarz-Schilling"="#668B8B",
                    "Lajcak"="#B22222",
                    "Ashdown"="#36648B")
...
plot.gov <- Bonn.joy %>%
  ...
  theme(legend.position = "none",
        ...
        strip.background = element_rect(fill="white",
                                        color="white"),
        panel.grid.minor.x = element_blank())+
  scale_x_continuous(breaks=seq(min(Bonn.joy$year),2018,1), 
                     labels = paste(substring(as.character((seq(1997,2018))),3)),
                     limits=c(1997,2018))+
  scale_y_continuous(limits=c(0,40))+
  scale_fill_manual(values=mandate.colors)+
  facet_grid(area2 ~ phase, labeller = label_wrap_gen(width = 25, multi_line = TRUE))
...
plot.recal <- Bonn.joy %>%
  ...
  #annotate("rect", xmin=2006,xmax=+Inf,ymin=0, ymax=40, fill="#E2F0F0B3", alpha=0.5)+
  geom_bar(aes(fill=HR), stat="identity", position="stack")+
  labs(caption="graph:author, data:OHR.int")+
  theme_thesis()+
  theme(legend.position = "bottom",
        legend.justification = "right",
        legend.key.width = unit(0.3, "cm"),
  ...
  scale_y_continuous(limits=c(0,40))+
  scale_fill_manual(values=mandate.colors)+
  facet_grid(area2 ~ phase, labeller = labeller(.rows=label_wrap_gen(width = 25, multi_line = TRUE)))+
...
library(patchwork)
patchwork::wrap_plots(list(plot.sanct, plot.gov, plot.recal))+
  plot_layout(ncol=1, nrow=3, heights=c(2,4,3),
              guides=)+
  plot_annotation(title="Bonn Powers as different means of intervention",
                  subtitle = "Number and type of Bonn Power decisions per year and mandate holder")


folder <-"/graphs/"
time <- format(Sys.time(),"%Y%m%d-%H%M%S")
filename <-"-combined.graph.png"
ggsave(paste0(wdr, folder,time,filename), 
       width=16, height=20, unit="cm", 
       type = "cairo")
