...
list.of.packages <- c("tidyverse","readit","hrbrthemes","skimr","lubridate","XML","UpSetR")
...
res96 <- readxl::read_excel("data/Electoral Results Bosnia.xlsx",sheet="HoR 1996")
...
res98_00 <- readxl::read_excel("data/Electoral Results Bosnia.xlsx",sheet="HoR 1998 2000")
...
res02 <- readxl::read_excel("data/Electoral Results Bosnia.xlsx",sheet="HoR 2002")
...
res06_14 <- readxl::read_excel("data/Electoral Results Bosnia.xlsx",sheet="HoR 2006 - 2014")
...
hor18 <- readxl::read_excel("data/ElectionHOR2018.xlsx") %>% 
  mutate(date=as.Date(date))
...
hor <- bind_rows(hor96, hor98, hor00, hor02, hor06_14, hor18)
...
party.names <- readxl::read_excel("data/Electoral Results Bosnia.xlsx",
                                  sheet="party.names2", col_types=c("text"))
...
party.ethnicity <- party.names %>% 
  distinct(name.unified2, ethnicity) %>% 
  filter(!is.na(name.unified2))

check <- party.names %>% 
  distinct(name.unified2, ethnicity) %>% 
  group_by(name.unified2) %>% 
  summarise(nobs=n(),
            ethnicities=paste(ethnicity, collapse=", ")) %>% 
  filter(nobs>1)


#adds ethnicity
hor2 <- left_join(hor, 
                  party.ethnicity %>% select(name.unified2, ethnicity), 
                  by=c("party.standard"="name.unified2")) %>% 
  ungroup()
...
#parties with seats without ethnicity
check<- hor2 %>% 
  filter(seats>0 & is.na(ethnicity))

#all parties with seats have ethnicity assinged and unified party name
write_xlsx(hor2, "data/BiH_HoR_consolidated.xlsx")



# >> check for non-matched parties ----------------------------------------

non.matched<- as.data.frame(unique(d$party.original))
length(unique(d$party.original))

writexl::write_xlsx(non.matched, "data/NonMatchedNames.xlsx")
...
eth.coal <- readxl::read_excel("data/Electoral Results Bosnia.xlsx",sheet="UniquePartyNamesSeats")
...
ethnicity.color <- c("B"="forestgreen","S"="red3","C"="blue","M"="orange")
...
n.parties.ethnicity.plot <- n %>%
  ...
  geom_step(aes(color=ethnicity), size=0.5)+
  labs(x="year",y="seats",
       title=paste0("Number of parties in House of\nRepresentatives per ethnic group"),
       subtitle="1 coalition counted as 1 party",
       caption="graph: author; data:www.izbori.ba")+
  scale_x_continuous(breaks=c(1996,1998,2000,2002,2006,2010,2014), labels=substr(unique(n$year),3,4)) +
  scale_color_manual(values=ethnicity.color)+
  scale_y_continuous(limits = c(0,5), minor_breaks = NULL)+
  theme_thesis()+
  theme(legend.position="none", 
    #    plot.margin=unit(c(0,3,0,0), "cm"),
        panel.grid.minor.x = element_blank())+
  facet_wrap(ethnicity~., labeller=ethnicity.labeller)
               
               
               
ethnicity.labeller <- labeller(ethnicity=c("S"="Serb", "C"="Croat", "B"="Bosniak", "M"="multi-ethnic"))  
...
library(formattable)
# n.tab <- n %>% 
#   select(year, ethnicity, n.parties) %>% 
#   spread(key=year, value=n.parties)
# 
# formattable(n.tab, list(area(col=2:8, "1996"=color_bar(color="lightblue"))))
...
time <- format(Sys.time(),"%Y%m%d-%H%M%S")
scriptname <- "BosniaHoR"
plotname <-"n.parties.ethnicity.plot.png"
ggsave(paste(folder,time,scriptname,plotname, sep="-"), width=7, height=7, unit="cm")  
...
time <- format(Sys.time(),"%Y%m%d-%H%M%S")
scriptname <- "BosniaHoR"
plotname <-"parties.col.facet.ethnicity.plot.png"
ggsave(paste(folder,time,scriptname,plotname, sep="-"), width=15, height=7)  
...
time <- format(Sys.time(),"%Y%m%d-%H%M%S")
scriptname <- "BosniaHoR"
plotname <-"parties.col.facet.yar.plot .png"
ggsave(paste(folder,time,scriptname,plotname, sep="-"), width=15, height=7)  


# >> Gini Seats in HOR per Ethic groups (WRONG)---------------------------------------------------
## this is WRONG; calculation requires zeros for parties which did not get a seat; otherwise full concentration
## treated as full equality when there is only one 1 party;
...
gini.seats <- hor %>%
  ...
  group_by(year, ethnicity) %>%
  summarize(gini.seats=reldist::gini(seats.year))
...
comp.hor.area.plot <- comp.hor.area %>%
  ...
  scale_x_continuous(breaks=c(1996,1998,2000,2002, 2006,2010,2014)) +
  scale_y_continuous(breaks=c(0,10,20,30,42))+
  scale_fill_manual(values=c("orange","darkgreen","darkblue","darkred","orange"), labels=c("Multiethnic/'Civic'","Bosniak","Croat","Serb"))+
  labs(title="Composition of House of Representatives per seats",
       subtitle="Categorized by ethnic affiliation",
...
scriptname <- "BosniaHoR"
plotname <-"comp.hor.plot.png"
ggsave(paste(folder,time,scriptname,plotname, sep="-"), width=15, height=10,unit="cm")  
...
tree.plot <- tree %>%
 ...
 geom_treemap()+
  ...
  scale_fill_manual(values=c("darkgreen","darkblue","darkred","orange","grey"),
                    labels=c("Bosniak","Croat","Serb","Multiethnic/'Civic'"))+
  labs(title="Ethnic segmentation of parties as per seats in House of Representatives",
       subtitle="changes largely within segments",
       caption="Data: Central Election Commission BiH")+
  facet_wrap(~year)
...
time <- format(Sys.time(),"%Y%m%d-%H%M%S")
scriptname <- "BosniaHoR"
plotname <-"tree.plot.png"
ggsave(paste(folder,time,scriptname,plotname, sep="-"), width=25, height=12, unit="cm")  
...
hor.res.plot  <- ggplot(hor.res,
      ...
      scale_color_manual(values=c("darkgreen","darkblue","darkred","orange","grey"), labels=c("Bosniak","Croat","Serb","Multiethnic/'Civic'"))+
 scale_y_continuous(breaks=c(0,10,20,30,40,50,60)) +
 scale_x_discrete(breaks = hor.res$order,labels = hor.res$party.abbrev, expand = c(0.1,.1)) +
 labs(title="Percentage of votes in elections to House of Representatives",
       subtitle="Categorized by ethnic affiliation and entity; only parties which won seats",
       caption="Data:Central Election Comission BiH")+
  theme_minimal()+
  theme(legend.position="bottom", legend.title = element_blank(),
        axis.title=element_blank(),
  ...
  #facet_wrap(year~entity, labeller=label_parsed, scales="free_y",drop=TRUE, ncol=2)+coord_flip() 
  facet_wrap(entity~year, labeller=label_parsed, scales="free_x",drop=TRUE, nrow=2) 
...
time <- format(Sys.time(),"%Y%m%d-%H%M%S")
scriptname <- "BosniaHoR"
plotname <-"hor.res.plot.png"
ggsave(paste(folder,time,scriptname,plotname, sep="-"), width=24, height=15, unit="cm")  
...
library(formattable)
formattable(n.parties, 
            list(n.parties.run=proportion_bar()))
...
# >> Number of Effective Parties (laakso/taagepera) -----------------------

effective.n<- hor %>%
    group_by(year, party.original) %>%
    summarise(votes.y=sum(votes),
              seats.y=sum(seats)) %>%
    mutate(votes.y.all=sum(votes.y),
           seats.y.all=sum(seats.y)) %>%
    mutate(votes.perc=votes.y/sum(votes.y),
           seats.perc=seats.y/sum(seats.y))%>%
    mutate(p=votes.perc^2,
           p.seats=seats.perc^2)%>%
    group_by(year)%>%
    summarise(n.eff=round(1/sum(p),2),
              n.eff.seats=round(1/sum(p.seats),2)) %>% 
    ungroup()

  
  

# >> Plot Number of Effecitve Parties -------------------------------------

effective.n.plot <- effective.n %>%
    gather(index, number, n.eff, n.eff.seats)%>%
    ggplot(.,aes(year,number))+
    geom_point(aes(color=index))+
    geom_line(aes(color=index))+ 
    scale_x_continuous(breaks=c(1996,1998,2000,2002, 2006,2010,2014)) +
    scale_color_manual(labels=c("based on votes","based on seats"), values =c("darkred","darkblue"))+
    labs(title="Number of effective parties in House of Representatives",
       subtitle="based on votes and seats; Laakso/Taagepera (1979)",
       caption="effective.n.plot",
       x="",y="")+
  theme_minimal()+ 
  theme(legend.position="bottom", legend.title = element_blank(),
        axis.title.x=element_blank(),
...
print(effective.n.plot )


folder <-"graphs/"
time <- format(Sys.time(),"%Y%m%d-%H%M%S")
scriptname <- "BosniaHoR"
plotname <-"effective.n.png"
ggsave(paste(folder,time,scriptname,plotname, sep="-"), width=21, height=10, unit="cm")    



# >> Number of effective parties only those with seats in HoR -----------------------

df<- hor %>% 
  ungroup() %>% 
  filter(seats > 0) %>% 
  group_by(date, party.standard, ethnicity) %>% 
  summarise(votes.party=sum(votes),
            seats.party=sum(seats)) %>% 
  group_by(date, ethnicity) %>% 
  mutate(votes.total.ethnic=sum(votes.party),
         seats.total.ethnic=sum(seats.party)) %>% 
  mutate(share.votes.ethnic.2=(votes.party/votes.total.ethnic)^2,
         share.seats.ethnic.2=(seats.party/seats.total.ethnic)^2) %>% 
  summarise(eff.n.votes=1/sum(share.votes.ethnic.2),
            eff.n.seats=1/sum(share.seats.ethnic.2))
  
  
df %>% 
  gather(key="type", value="value", -date, -ethnicity) %>% 
  ggplot()+
  ...
  geom_point(aes(x=date, 
                y=value,
                color=type))+
  scale_x_date(date_labels="%y", breaks=as.Date(unique(df$date)))+
  scale_y_continuous(limits=c(0,4))+
  facet_grid(ethnicity ~ type)
...
vol.seg <- hor %>%
    ...
    ungroup()%>%
    mutate(share=y.eth.total/y.total)%>%
    group_by(ethnicity)%>%
    mutate(vol.eth.annual=share-lag(share))%>%
    group_by(year)%>%
    summarise(vol=sum(abs(vol.eth.annual))/2)


# >>> plot inter-seg volatility (seats)----------------------------------
...
time <- format(Sys.time(),"%Y%m%d-%H%M%S")
scriptname <- "BosniaHoR"
plotname <-"vol.seg.plot.png"
ggsave(paste(folder,time,scriptname,plotname, sep="-"), width=21, height=10, unit="cm")  


# >> Intra-Segment Volatility based on seats---------------------------------------------
#potential problem: volatility distorted because of pre-election coalitions; would
#more valid if seats are counted as eventually distributed among members of these pre-
...
vol.intra <- hor %>%
  ...
  group_by(year, ethnicity)%>%
  mutate(sum.y=sum(seats.y))%>%
  mutate(perc=round(seats.y/sum.y,2))%>%
  group_by(party.standard)%>%
  mutate(perc.lag=lag(perc))%>%
  mutate(perc.lag=ifelse(is.na(perc.lag),0, perc.lag))%>%
  mutate(diff.perc=perc-perc.lag)%>%
  filter(year>1996)%>%
  group_by(ethnicity, year)%>%
  summarise(vol=sum(abs(diff.perc))/2) 

# >> append intra-seg and inter-seg volatility ---------------------------

vol.seg$type <- "inter-segment"
vol.intra <- rename(vol.intra, type=ethnicity)
vol <- bind_rows(vol.intra,vol.seg[vol.seg$year>1996,])
vol$type <- as.factor(vol$type)
levels(vol$type) <- c("intra-Bosniak", "intra-Croat", 
                       "inter-segment",
                       "inter-multi-ethnic",
                       "inter-Serb")

vol$type = factor(vol$type, levels=c('intra-Bosniak',
                                     'intra-Croat',
                                     'inter-Serb',
                                     'inter-multi-ethnic',
                                     'inter-segment'))

# >>> Plot volatility - intra & inter segment -----------------------------
...
vol.plot <-  vol %>%
  ...
  geom_rect(data = subset(vol,type == 'inter-segment'),fill = "lightgrey",
            aes(xmin = -Inf,xmax = Inf,
            ymin = -Inf,ymax = Inf), alpha = 0.1) +
  scale_x_continuous(breaks=c(1996,1998,2000,2002, 2006,2010,2014),
                     labels=c("96","98","00","02","06","10","14")) +
  scale_color_manual(values =c("darkgreen","darkblue","darkred","orange","black"))+
    labs(title="Electoral volatility between and within ethnic segments",
       subtitle="Based on seats in House of Representatives",
       caption="Data: Central Election Commission BiH",
       x="",y="")+
  theme_minimal()+ 
  theme(legend.position="none", legend.title = element_blank(),
...
time <- format(Sys.time(),"%Y%m%d-%H%M%S")
scriptname <- "BosniaHoR"
plotname <-"vol.plot.png"
ggsave(paste(folder,time,scriptname,plotname, sep="-"), width=16, height=10, unit="cm")  



# >> Time to form government -------------------------------------------------

CoM <- readxl::read_excel("data/Electoral Results Bosnia.xlsx",sheet="CoM")

gov.start <- CoM %>%
  ungroup() %>% 
  select(period, date.start)%>%
  group_by(period)%>%
  summarise(gov.start=min(date.start))%>%
  mutate(cycle.year=substr(period,1,4)) %>% 
  #mutate(cycle.year=gsub("\\-.*","",period))%>%
  mutate(gov.start=as.Date(format(gov.start,"%Y-%m-%d")))

election.dates  <- hor %>%
  select(date)%>%
  rename(election.date=date)%>%
  mutate(cycle.year=format(election.date, "%Y"))%>%
  distinct()
  
gov.formation <- full_join(election.dates, gov.start, by="cycle.year")
gov.formation$duration <- round(difftime(gov.formation$gov.start,gov.formation$election.date, units="weeks"),0)
  

# >>> plot Time to form government ----------------------------------------

gov.formation.plot <-  gov.formation %>%
  ungroup()%>%
  ggplot(.,aes(cycle.year, as.numeric(duration)))+
  geom_bar(stat="identity", fill=c("darkblue"))+
  labs(title="Duration of formation of Council of Ministers",
       subtitle="in weeks",
       caption="gov.formation.plot",
       x="Election year",y="number of weeks")+
  theme_minimal()+ 
  theme(legend.position="none", legend.title = element_blank(),
        axis.title.x=element_blank(),
        panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(),
        panel.grid.minor.y = element_blank())
print(gov.formation.plot)

folder <-"graphs/"
time <- format(Sys.time(),"%Y%m%d-%H%M%S")
scriptname <- "BosniaHoR"
plotname <-"gov.formation.plot.png"
ggsave(paste(folder,time,scriptname,plotname, sep="-"), width=30, height=10, unit="cm")  
...
time <- format(Sys.time(),"%Y%m%d-%H%M%S")
scriptname <- "BosniaHoR"
plotname <-"votes.seats.concentration.png"
ggsave(paste(folder,time,scriptname,plotname, sep="-"), width=15, height=7)  


#continue here; where does seats come from?
